...
def make_withdraw(balance, password):
    ...
    """
    "*** YOUR CODE HERE ***"
    error_code, locked = [], False
    def withdraw(amount, s):
        nonlocal balance, password, error_code, locked
        if locked:
            return f"Too many incorrect attempts. Attempts: {error_code}"
        if password == s:
            if amount > balance:
                return 'Insufficient funds'
            else:
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    if type(check) == str:
        return check
    def joint_withdraw(amount, psd):
        nonlocal old_pass, new_pass
        return withdraw(amount, old_pass if psd == new_pass else psd)
